...
import { FiliereFormData, DepartmentFormData, AnneeAcademiqueFormData, ClasseFormData, OptionFormData } from '../../dataTypes/data';
...
interface SearchClasseFormProps {
    title : string;
    subtitle : string;
    onSubmit: (classe: ClasseFormData) => void;
}

const SearchClasseForm: React.FC<SearchClasseFormProps> = ({ onSubmit, title, subtitle }) => {
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data" className='mt-0 mb-6 border-b-4 border-blue-300 bg-slate-100 p-4'>
            ...
            {anneeAcademique &&
                <h5 className='mb-4 italic text-sm text-gray-500'>
                   {subtitle} {new Date(anneeAcademique.date_debut).getFullYear()}/{new Date(anneeAcademique.date_fin).getFullYear()}
                </h5>
            }
            {!anneeAcademique &&
...
export default SearchClasseForm;
